...
# sync and stack rasters
  for(i in 1:length(rasters)){
    if("dem" %in% names(rasters[[i]])){ # setting dem as a reference
      dem <- rasters[[i]] # dem as reference
    }
    }
...
# rasterize aoi and Mask rasters
  ...
  # r <- raster()
  # aoi <- rasterize(aoi, r)
  # aoi <- spatial_sync_raster(aoi, grid)
...
# Convert to back to Spatial Grid Dataframe
...
# extracting values by points
  grid_val <- over(points, sgdf_final)
...
# ------------------------------------------------------------------------------
# binding response var and predictor data
...
# variables
inTrain_N <- caret::createDataPartition(y = grid_data_N$N, p = .75, list = FALSE)
...
# Exporting the prediction as raster
  pred_grid_N_sp <-
